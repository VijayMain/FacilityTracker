...
@WebServlet("/SixSigma_Control")
public class SixSigma_Control extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			if (ServletFileUpload.isMultipartContent(request)) {
				...
				List fileItemsList; 
					...
					while (it.hasNext()) {
						...
						if (fileItemTemp.isFormField()) {
							...
							if (fieldName.equalsIgnoreCase("applicable_act")) {
								vo.setApplicable_act(Integer.valueOf(fieldValue));
							}
							...
							if (fieldName.equalsIgnoreCase("control_filename")) {
							...
							if (fieldName.equalsIgnoreCase("control_filename"+vo.getHis_id())) {
								file_stored = fileItem.getName(); 
								vo.setAnalyze_filename(FilenameUtils.getName(file_stored)); 
								file_Input = new DataInputStream(fileItem.getInputStream()); 
								vo.setAnalyze_attach(file_Input); 
				...
				dao.controlDetails(session,vo,response); 
			}
		} catch (Exception e) {
			e.printStackTrace();
		}

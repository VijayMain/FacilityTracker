...
@WebServlet("/SixSigma_Attach_defineControl")
public class SixSigma_Attach_defineControl extends HttpServlet {
		...
		try {
			...
			**********************************************************************************************************/
			if (ServletFileUpload.isMultipartContent(request)) {
				...
				List fileItemsList; 
					...
					while (it.hasNext()) {
						...
						if (fileItemTemp.isFormField()) {
							...
							if (fieldName.equalsIgnoreCase("def_probID")) {
								vo.setProblem_id(Integer.parseInt(fieldValue)); 
							}
							if (fieldName.equalsIgnoreCase("namePhase")) {
								vo.setDef_probID(Integer.parseInt(fieldValue)); 
							}
						} else {
							// *************************************************************************************************************
							// IF FILE inputs === >
							// *************************************************************************************************************
							String file_stored = null,nameCorrect=null;
							
							fileItem = fileItemTemp;
							fieldName = fileItem.getFieldName();
							fieldValue = fileItem.getString();
							
							if (fieldName.equalsIgnoreCase("file_attach")) {
								file_stored = fileItem.getName(); 
								nameCorrect = FilenameUtils.getName(file_stored);
								nameCorrect.replaceAll("'","");
								nameCorrect.replace("\"", ""); 
								vo.setAnalyze_filename(nameCorrect); 
				...
				dao.define_Attach(session,vo,response); 
			}
		} catch (Exception e) {
			e.printStackTrace();
		}

...
@WebServlet("/Master_DataMM60_Upload")
public class Master_DataMM60_Upload extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			if (ServletFileUpload.isMultipartContent(request)) {
				...
				while (it.hasNext()) {
					...
					if (fileItemTemp.isFormField()) {
						...
						if (fieldName.equalsIgnoreCase("reason")) {
							bean.setReason(fieldValue);
						}
						if (fieldName.equalsIgnoreCase("hsn")) {
							bean.setHsn(fieldValue);
						}
					} else {
						// *************************************************************************************************************
						// IF FILE inputs === >
				...
				if (bean.getFile_blob() != null && bean.getContact()!="" && bean.getHsn()=="") {
					dao.attach_File(response, bean, session);
				}else{
					dao.attachHSN_File(response, bean, session);
				}
			}
		} catch (Exception e) {
			e.printStackTrace();

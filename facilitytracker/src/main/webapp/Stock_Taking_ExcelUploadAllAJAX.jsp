...
try{ 
	...
	String fileNM="";
	if(excel==0){
		fileNM = "All Data Punch File";
%>
<span id="getcheckUploadAll">  
<%	  
	}else{
		fileNM = "All Data Punch File [Extra]";
%>
<span id="getcheckUpload_ExtraAll"> 
<%		
	}
	    ...
	    Label label12=new Label(6,  0, "Entry Qty Sum",cellFormat); writableSheet.addCell(label12);
	    Label label9=new Label(7,  0, "Tag No",cellFormat); writableSheet.addCell(label9);
	    Label label13=new Label(8,  0, "Batch No.",cellFormat); writableSheet.addCell(label13); 
	 	    ...
	 	    Label label8=new Label(4,  0, "Entry Qty",cellFormat); writableSheet.addCell(label8);
	 	   	Label label12=new Label(5,  0, "Entry Qty Sum",cellFormat); writableSheet.addCell(label12);
	   ...
	   // System.out.println(data_entryPLant+" = " +  fiscal_year + " = " + storage_location  + "  = "  + " = " + stockType);
	    String zerocnt="",query_name = "", matCode="",matDescr="",MatTye="";
	    
	    if(excel==0){
	    query_name = "select * from stocktaking_summary where enable=1 and fiscal_year='"+String.valueOf(fiscal_year)+"' and Plant='"+data_entryPLant+"' and storage_loc='"+storage_location+"' "+
				" and enable=1 and stocktype_id="+stockType+" order by id,sno";	
	    	
	 	}else{
	 	query_name = "select * from stocktaking_summary_extra where enable=1 and fiscal_year='"+String.valueOf(fiscal_year)+"' and Plant='"+data_entryPLant+"' and storage_loc='"+storage_location+"' "+
				" and enable=1 and stocktype_id="+stockType;	
	 	}
		...
		while(rs_check.next()){
				...
				matCode=rs_check.getString("material_no");
				matDescr=rs_check.getString("material_desc");
				MatTye=rs_check.getString("mat_type"); 
		...
		Label entryQtysum =new Label(col, row, rs_check.getString("entry_qty_string"),cellleftformat);writableSheet.addCell(entryQtysum);col++; 

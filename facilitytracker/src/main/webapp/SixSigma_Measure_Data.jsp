...
function validatenumerics(key) { 
  ...
  function decision_call(val) {
	...
	function selected_checkbox(cnt,id) {
		if(document.getElementById(cnt).checked==true){
				...
				xmlhttp1.open("POST","SixSigma_toolsUsed.jsp?cnt=" + cnt + "&id=" + id +"&select=1", true); 
				xmlhttp1.send();  
			 /************************************************************************************************/
		}else{
				var xmlhttp1;
				...
				xmlhttp1.open("POST","SixSigma_toolsUsed.jsp?cnt=" + cnt + "&id=" + id +"&select=0", true); 
				xmlhttp1.send();
				/************************************************************************************************/	 
		}
	};
	...
	function numericEvent(val,input) {			    
		...
		var sum = parseInt(defineScore) + parseInt(measureScore);
		...
		if (val<input || input==0) {
		   ...
		   alert("Wrong Input");
			...
			sum = parseInt(defineScore)+parseInt(input);
			document.getElementById('logScore').value = sum;
		}
	}
	...
	function saveSSVData(probID,phaseID) {
		var ssv_text = document.getElementById("ssv_text").value; 
		var ssv_measure = document.getElementById('ssv_measure').value;  
		var ssv_variation = document.getElementById('ssv_variation').value; 
		var ssvImage = document.getElementById('ssvImage');
		
		if(ssv_text!="" && ssv_measure!="" && ssv_variation!=""){
			...
			ssvImage.style.visibility = "hidden";
				...
				xmlhttp.onreadystatechange = function() {
					if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
						...
						document.getElementById('ssv_variation').selectedIndex = 0;  
						document.getElementById('ssv_measure').selectedIndex = 0;
						document.getElementById('ssv_text').value=''; 
						ssvImage.style.visibility = "visible";
					}
				};
			  	xmlhttp.open("POST", "SixSigma_MeasureSSV.jsp?probID="+probID +"&phaseID="+  phaseID  +"&ssv_variation="+ ssv_variation +"&ssv_measure="+ 
			  			ssv_measure +"&ssv_text="+  ssv_text, true);  
				xmlhttp.send();
		}else{
	...
	function validateForm() {
		...
		if(flag==false){
			alert("Please select Tools used in the Measure Phase");
...
try{
	...
	PreparedStatement ps_check = null, ps_check1=null,ps_local=null,ps_sql=null;
	ResultSet rs_check = null,rs_check1=null,rs_local=null,rs_sql=null; 
...
<section class="wrapper">
   <!--overview start-->
    <div class="row">
        <%
        String measure="measure_data";
        %>
        <jsp:include page="SixSigma_Header.jsp" >
  		<jsp:param name="define" value="<%=measure %>" />
		</jsp:include>
        </div>
<!-- ********************************************************************************************************************* -->
<div> 
...
int company_id=0, projectLead_id=0,phaseSubmit=0,phaseID=0;
PreparedStatement ps_data=null,ps_data2=null,ps_data1=null,ps_checkMeasure=null;
ResultSet rs_data=null,rs_data2=null,rs_data1=null,rs_checkMeasure=null;
...
<table class="gridtable" width="100%">
  <tr>
    ...
    <td align="center" style="font-size: 12px;font-weight: bold;background-color: #a59eff">MEASURE PHASE</td>
      	<%
  		if(request.getParameter("success")!=null){
		%>
		<td align="center" style="font-size: 12px;font-weight: bold;background-color: #06a415;color: white;">
		<a href="SixSigma_Measure_Data.jsp?problem_id=<%=problem_id%>" style="color: white;"><%=request.getParameter("success") %></a>
		</td>
        <%
		...
		<a href="SixSigma_Measure_Data.jsp?problem_id=<%=problem_id%>"  style="color: white;"><%=request.getParameter("statusNop") %></a>
		</td>  
	    <%
		}
		%> 
...
while(rs_sql.next()){
	...
	String  measure_days ="",analyze_days ="",improve_days ="",control_days ="";
	int  measure_lbl =0,analyze_lbl =0,improve_lbl =0,control_lbl =0;
...
<img src="img/LogHere.png" title="History" style="cursor: pointer;height: 25px;">
  ...
  <tr>
   ...
   String define_days="";
    ...
    rs_check1 = ps_check1.executeQuery();
    while(rs_check1.next()){
    	define_days = sdf.format(rs_check1.getDate("plan_endDate"));
    %>
  <tr>
  ...
  <tr>
    ...
    <% 
    int defineScore=0,measureScore=0,totalSum=0;
	...
	ps_data = con_master.prepareStatement("select actual_score from rel_pt_dateTimeline where enable=1 and phase_id="+phaseID+" and problem_id="+problem_id);
	rs_data = ps_data.executeQuery();
	if(rs_data.next()){
		measureScore = rs_data.getInt("actual_score"); 
	} 
	totalSum = defineScore + measureScore;
	%>
    ...
    rs_check1 = ps_check1.executeQuery();
    if(rs_check1.next()){
    	ps_data = con_master.prepareStatement("select actual_score,act_startDate,act_endDate from rel_pt_dateTimeline where enable=1 and (phase_id=3 or phase_id=4) and problem_id="+problem_id);
    	rs_data = ps_data.executeQuery();
    	if(rs_data.next()){
   	...
   	%>
    	    <input type="text" name="input_value" id="input_value" value="<%=inputValue %>" onkeypress="return validatenumerics(event);" onkeyup="numericEvent(<%= measure_lbl %>,this.value)" maxlength="3" minlength="1" class="form-control" style="height: 23px !important;width: 60px;color: black;font-weight: bold;text-align: right;" required>
    	    <%		
    	}else{
    	ps_data = con_master.prepareStatement("select id from rel_pt_reviewer where enable=1 and u_id="+uid);
		rs_data = ps_data.executeQuery();
		if(rs_data.next()){
		    %>
		    <input type="text" name="input_value" id="input_value"  onkeypress="return validatenumerics(event);" onkeyup="numericEvent(<%= measure_lbl %>,this.value)" maxlength="3" minlength="1" class="form-control" style="height: 23px !important;width: 60px;color: black;font-weight: bold;text-align: right;" required>
		    <%
			}
   		 }
    	}
    ...
    <td style="color: black;" align="right"><%=define_days %></td>
    <td style="color: black;" align="right"><%=measure_days %></td>
...
<form action="SixSigma_MeasureControl" method="post" enctype="multipart/form-data" onSubmit="return validateForm();">
<input type="hidden" name="editMeasure" id="editMeasure" value="1">
<!-- ***************************************************************************************************************************** -->
<!-- ********************************************** History Measure Data *********************************************************** -->
<%
ps_checkMeasure = con_master.prepareStatement("select * from rel_pt_measurePhase where enable=1 and problem_id="+problem_id+" and phase_id="+phaseID);
rs_checkMeasure = ps_checkMeasure.executeQuery();
if(rs_checkMeasure.next()){
%>
<input type="hidden" name="measureID" id="measureID" value="<%=rs_checkMeasure.getInt("id")%>">
...
<table class="gridtable" width="100%">
  <tr>
    <td style="font-size: 12px;font-weight: bold;background-color: #a59eff;" align="center">Measure Details</td>
    <td style="font-size: 12px;font-weight: bold;background-color: #a59eff;" align="center">Update</td>
    <td style="font-size: 12px;font-weight: bold;background-color: #a59eff;" align="center">Attach Ref(If Any)</td>
  </tr>
  <tr>
    <th>Suspected Scientific reason(s) for the problem</th>
    <td colspan="2" style="font-size: 12px;color: black;">
    <%
    if(!rs_checkMeasure.getString("scientific_reason").equalsIgnoreCase("")){
    %> 
     <textarea class="form-control" readonly="readonly" name="scientific_reason" id="scientific_reason" rows="1" cols="30" onkeypress="return checkQuote();" style="color: black;font-size: 12px;"><%= rs_checkMeasure.getString("scientific_reason") %></textarea>
   <%	
    }else{
    %>
    <textarea class="form-control" name="scientific_reason" id="scientific_reason" rows="1" cols="30" onkeypress="return checkQuote();" style="color: black;font-size: 12px;"></textarea>
    <%	
    }
    %>
    </td>
  ...
  <tr>
    <th>Suspected Last Manufacturing process where the problem is generated</th>
     <td colspan="2" style="font-size: 12px;color: black;"> 
     <%
    if(!rs_checkMeasure.getString("problem_lastManProcess").equalsIgnoreCase("")){
    %>
    	<textarea class="form-control" readonly="readonly"  name="problem_lastManProcess" id="problem_lastManProcess" rows="1" cols="30" onkeypress="return checkQuote();" style="color: black;font-size: 12px;"><%= rs_checkMeasure.getString("problem_lastManProcess") %></textarea> 
    <%	
    }else{
    %> 
   		<textarea class="form-control" name="problem_lastManProcess" id="problem_lastManProcess" rows="1" cols="30" onkeypress="return checkQuote();" style="color: black;font-size: 12px;"></textarea> 
   	<%
    }
   	%>	
   	</td>
  ...
  <tr>
    <th>Process stages where the Problem is detected</th>
     <td colspan="2" style="font-size: 12px;color: black;"> 
    <%
    if(!rs_checkMeasure.getString("problem_processStages").equalsIgnoreCase("")){
    %>
    <textarea class="form-control" readonly="readonly"  name="problem_processStages" id="problem_processStages" rows="1" cols="30" onkeypress="return checkQuote();" style="color: black;font-size: 12px;"><%=rs_checkMeasure.getString("problem_processStages") %></textarea>  
    <%	
    }else{
    %> 
    <textarea class="form-control" name="problem_processStages" id="problem_processStages" rows="1" cols="30" onkeypress="return checkQuote();" style="color: black;font-size: 12px;"></textarea> 
   	<%
    }
   	%>
     </td>
  ...
  <tr>
    <th>Month in which the rejection is minimum</th>
    
<%
if(rs_checkMeasure.getInt("year_mincnt")!=0){
    String[] months = new DateFormatSymbols().getMonths();  
    java.util.Calendar calendar = java.util.Calendar.getInstance(); 
    int year = calendar.get(java.util.Calendar.YEAR);
    int currMonth = calendar.get(java.util.Calendar.MONTH);
%>
<td style="font-size: 12px;color: black;">
	<select name="month_minRejection" id="month_minRejection" class="form-control" style=" font-size: 12px !important;color: black;text-align: left !important;">
    <option value="<%= rs_checkMeasure.getInt("month_mincnt")%>"><%= months[rs_checkMeasure.getInt("month_mincnt")].toString()%></option>
  	</select>
</td> 	 
<td style="font-size: 12px;color: black;">
<select name="year_minRejection" id="year_minRejection" class="form-control" style="font-size: 12px !important;color: black;text-align: left !important;">
    <option value="<%=rs_checkMeasure.getInt("year_mincnt")%>"><%=rs_checkMeasure.getInt("year_mincnt")%></option> 
</select>
</td>
<%
}else{
...
<select name="month_minRejection" id="month_minRejection" class="form-control" style=" font-size: 12px !important;color: black;text-align: left !important;">
    ...
    <option value="<%= currMonth%>"><%= months[currMonth].toString()%></option>
    <%
    for (int i = 0; i < months.length-1; i++) {
        if(currMonth!=i){
    %>
    <option value="<%= i%>"><%= months[i].toString()%></option>
    <%
        }
    }
    %>
...
%>   
  ...
  <tr>
    <th>Month in which the rejection is maximum</th>
     
   
   
   <%
if(rs_checkMeasure.getInt("year_maxcnt")!=0){
    String[] months = new DateFormatSymbols().getMonths();  
    java.util.Calendar calendar = java.util.Calendar.getInstance(); 
    int year = calendar.get(java.util.Calendar.YEAR);
    int currMonth = calendar.get(java.util.Calendar.MONTH);
%>
<td style="font-size: 12px;color: black;">
	<select name="month_maxRejection" id="month_maxRejection" class="form-control" style=" font-size: 12px !important;color: black;text-align: left !important;">
    <option value="<%= rs_checkMeasure.getInt("month_maxcnt")%>"><%= months[rs_checkMeasure.getInt("month_maxcnt")].toString()%></option>
  	</select>
</td> 	 
<td style="font-size: 12px;color: black;">
<select name="year_maxRejection" id="year_maxRejection" class="form-control" style="font-size: 12px !important;color: black;text-align: left !important;">
    <option value="<%=rs_checkMeasure.getInt("year_maxcnt")%>"><%=rs_checkMeasure.getInt("year_maxcnt")%></option> 
</select>
</td>
<%
}else{
...
%>  
  ...
  <tr>
    <th>Minimum Rejection ppm</th>
    <td colspan="2" style="font-size: 12px;color: black;">
    <%
    if(rs_checkMeasure.getInt("min_rejectPPM")!=0){
    %>
<input type="text" id="min_rejectPPM" name="min_rejectPPM" readonly="readonly" value="<%=rs_checkMeasure.getInt("min_rejectPPM")  %>" class="form-control" onkeypress="return validatenumerics(event);" style="font-size: 12px !important;color: black;text-align: left !important;">
    <%
    }else{
    %>
<input type="text" id="min_rejectPPM" name="min_rejectPPM" class="form-control" onkeypress="return validatenumerics(event);" style="font-size: 12px !important;color: black;text-align: left !important;">    
    <%	
    }
    %>
    </td> 
  ...
  <tr>
    <th>Maximum Rejection ppm</th>
    <td colspan="2" style="font-size: 12px;color: black;">
    <%
    if(rs_checkMeasure.getInt("max_rejectPPM")!=0){
    %>
<input type="text" name="max_rejectPPM" id="max_rejectPPM" value="<%=rs_checkMeasure.getInt("max_rejectPPM") %>" readonly="readonly" class="form-control" onkeypress="return validatenumerics(event);" style="font-size: 12px !important;color: black;text-align: left !important;">
    <%
    }else{
     %>
<input type="text" name="max_rejectPPM" id="max_rejectPPM" class="form-control" onkeypress="return validatenumerics(event);" style="font-size: 12px !important;color: black;text-align: left !important;">
    <%	
    }
    %>
    </td>
  ...
  <tr>
    <th>Number of Machines used for doing the Suspected Last Manufacturing Process</th>
    <td colspan="2" style="font-size: 12px;color: black;">
    <%
    if(rs_checkMeasure.getInt("no_machinesUsed")!=0){
    %>
    <input type="text" name="no_machinesUsed" id="no_machinesUsed" value="<%=rs_checkMeasure.getInt("no_machinesUsed") %>" readonly="readonly" class="form-control" onkeypress="return validatenumerics(event);" style="font-size: 12px !important;color: black;text-align: left !important;">
    <%
    }else{
    %>
    <input type="text" name="no_machinesUsed" id="no_machinesUsed" class="form-control" onkeypress="return validatenumerics(event);" style="font-size: 12px !important;color: black;text-align: left !important;">
    <%
    }
    %>
    </td>
  ...
  <tr>
    <th>Number of Streams in the Suspected Last Manufacturing Process</th>
    <td colspan="2" style="font-size: 12px;color: black;">
    <%
    if(rs_checkMeasure.getInt("no_streams")!=0){
    %>
    <input type="text" name="no_streams" id="no_streams" value="<%=rs_checkMeasure.getInt("no_streams") %>" readonly="readonly" class="form-control" onkeypress="return validatenumerics(event);" style="font-size: 12px !important;color: black;text-align: left !important;">
    <%
    }else{
    %>
    <input type="text" name="no_streams" id="no_streams" class="form-control" onkeypress="return validatenumerics(event);" style="font-size: 12px !important;color: black;text-align: left !important;">
    <%
    }
    %>
    </td>
  ...
  <tr>
    ...
    <td colspan="2" style="font-size: 12px;color: black;"> 
    <%
    if(rs_checkMeasure.getInt("response")!=0){
    ps_check = con_master.prepareStatement("select * from rel_pt_masterData where id="+rs_checkMeasure.getInt("response"));
    rs_check = ps_check.executeQuery();
	%> 
	<select id="response" name="response" class="form-control" onchange="Getspecification()" style="font-weight:bold; font-size: 12px !important;color: black;text-align: left !important;">
   	<%
	while(rs_check.next()){
	%>
    ...
    ps_check = con_master.prepareStatement("select * from rel_pt_masterData where enable=1 and searchTerm='typeofResponse'");
    rs_check = ps_check.executeQuery();
    %>
	<select id="response" name="response" class="form-control" onchange="Getspecification()" style="font-weight:bold; font-size: 12px !important;color: black;text-align: left !important;">
    <option value=""> - - Select - - </option>
  ...
  <tr>
    <th>Specification(If Variable)</th>
    <td colspan="2" style="font-size: 12px;color: black;">
    <%
    if(!rs_checkMeasure.getString("specification").equalsIgnoreCase("")){		
    %>
    <span id="varSpec">
    <textarea class="form-control" name="specification" id="specification" rows="1" cols="30" onkeypress="return checkQuote();" style="color: black;font-size: 12px;" readonly="readonly"><%=rs_checkMeasure.getString("specification")  %></textarea> 
	</span>
    <%
    }else{
    %>
  ...
  <tr>
    <th>Are there any abnormalities in the Equipment condition found during Audit</th>
    <td style="font-size: 12px;color: black;">
    <%
    if(rs_checkMeasure.getInt("abnormal_equipmentCond")!=0){
    %>
    <% if(rs_checkMeasure.getInt("abnormal_equipmentCond")==1){ %> 
     <select id="abnormal_equipmentCond" name="abnormal_equipmentCond" class="form-control" style="font-weight:bold; font-size: 12px !important;color: black;text-align: left !important;">
     <option value="1">Yes</option>  
    </select>
    
    
    
    <% }else if(rs_checkMeasure.getInt("abnormal_equipmentCond")==2){ %> 
    <select id="abnormal_equipmentCond" name="abnormal_equipmentCond" class="form-control" style="font-weight:bold; font-size: 12px !important;color: black;text-align: left !important;">
	...
	<td style="font-size: 12px;color: black;">
	<%
	if(!rs_checkMeasure.getString("abnormal_eqCond_fileName").equalsIgnoreCase("")){
	%>
	<input type="hidden" name="abnormal_eqCond_fileName_flag" id="abnormal_eqCond_fileName_flag" value="avail">
	<a href="SixSigma_MeasureDocsDisplay.jsp?field=<%=rs_checkMeasure.getInt("id")%>&cond=abnormal_equipmentCond"><strong style="color: blue;font-weight: bold;"><%=rs_checkMeasure.getString("abnormal_eqCond_fileName") %></strong></a>
	<%
	}else{
	%>
	<input type="file" name="abnormal_eqCond_fileName" id="abnormal_eqCond_fileName" class="form-control" size="40" style="width:300px;padding-bottom:20px ;font-size: 12px !important;color: black;text-align: left !important;">
	<%	
	}
	%>	
    </td>
  ...
  <tr>
    <th>Are there any abnormalities in the Process parameters found during Audit</th>
    <td style="font-size: 12px;color: black;">
    <%
    if(rs_checkMeasure.getInt("abnormal_processParameters")!=0){
    %>
    <% if(rs_checkMeasure.getInt("abnormal_processParameters")==1){ %>
    <select id="abnormal_processParameters" name="abnormal_processParameters" class="form-control" style="font-weight:bold; font-size: 12px !important;color: black;text-align: left !important;" >
     <option value="1">Yes</option>  
    </select> 
    <% }else if(rs_checkMeasure.getInt("abnormal_processParameters")==2){%>
    <select id="abnormal_processParameters" name="abnormal_processParameters" class="form-control" style="font-weight:bold; font-size: 12px !important;color: black;text-align: left !important;" >
     <option value="2">No</option> 
    </select> 
    <% 
    ...
    <td style="font-size: 12px;color: black;">
    <%
    if(!rs_checkMeasure.getString("abnormal_processPara_filename").equalsIgnoreCase("")){
    %>
    <input type="hidden" name="abnormal_processPara_filename_flag" id="abnormal_processPara_filename_flag" value="avail">
	<a href="SixSigma_MeasureDocsDisplay.jsp?field=<%=rs_checkMeasure.getInt("id")%>&cond=abnormal_processParameters"><strong style="color: blue;font-weight: bold;"><%=rs_checkMeasure.getString("abnormal_processPara_filename") %></strong></a>
    <%
    }else{
    %>
    <input type="file" name="abnormal_processPara_filename" id="abnormal_processPara_filename" class="form-control" size="40" style="width:300px;padding-bottom:20px ;font-size: 12px !important;color: black;text-align: left !important;">
    <%	
    }
    %>
    </td>
  ...
  <tr>
    <th>Is the Rejection Trend for 6 months plotted</th>
    <td style="font-size: 12px;color: black;">
    <%
    if(rs_checkMeasure.getInt("rejection_trnd6Month")!=0){
    %>
     <% if(rs_checkMeasure.getInt("rejection_trnd6Month")==1){ %>
     <select id="rejection_trnd6Month" name="rejection_trnd6Month" class="form-control" style="font-weight:bold; font-size: 12px !important;color: black;text-align: left !important;" >
    	<option value="1">Yes</option>  
     </select> 
     <% }else if(rs_checkMeasure.getInt("rejection_trnd6Month")==2){%>
    <select id="rejection_trnd6Month" name="rejection_trnd6Month" class="form-control" style="font-weight:bold; font-size: 12px !important;color: black;text-align: left !important;" >
    	<option value="2">No</option> 
    </select> 
     <% 
    ...
    <td style="font-size: 12px;color: black;">
    <%
    if(!rs_checkMeasure.getString("rejection_trnd6Month_fileName").equalsIgnoreCase("")){
    %>
     <input type="hidden" name="rejection_trnd6Month_fileName_flag" id="rejection_trnd6Month_fileName_flag" value="avail">
	  <a href="SixSigma_MeasureDocsDisplay.jsp?field=<%=rs_checkMeasure.getInt("id")%>&cond=rejection_trnd6Month"><strong style="color: blue;font-weight: bold;"><%=rs_checkMeasure.getString("rejection_trnd6Month_fileName") %></strong></a>
    <%
    }else{
    %>
    <input type="file" name="rejection_trnd6Month_fileName" id="rejection_trnd6Month_fileName" class="form-control" size="40" style="width:300px;padding-bottom:20px ;font-size: 12px !important;color: black;text-align: left !important;">
    <%
    }
    %>
    </td>
  ...
  <tr>
    <th>Is Concentration chart applicable for the Project</th>
    <td>
    <%
    if(rs_checkMeasure.getInt("concentration_chart")!=0){
    %>
    <% if(rs_checkMeasure.getInt("concentration_chart")==1){ %>
    <select id="concentration_chart" name="concentration_chart" class="form-control" style="font-weight:bold; font-size: 12px !important;color: black;text-align: left !important;" >
    <option value="1">Yes</option>  
    </select>
    <% }else if(rs_checkMeasure.getInt("concentration_chart")==2){ %>
    <select id="concentration_chart" name="concentration_chart" class="form-control" style="font-weight:bold; font-size: 12px !important;color: black;text-align: left !important;" >
      <option value="2">No</option> 
    </select>
    <% 
    ...
    <td style="font-size: 12px;color: black;">
    <%
   if(!rs_checkMeasure.getString("concentration_chart_filename").equalsIgnoreCase("")){ 
    %>
    <input type="hidden" name="concentration_chart_filename_flag" id="concentration_chart_filename_flag" value="avail">
	<a href="SixSigma_MeasureDocsDisplay.jsp?field=<%=rs_checkMeasure.getInt("id")%>&cond=concentration_chart"><strong style="color: blue;font-weight: bold;"><%=rs_checkMeasure.getString("concentration_chart_filename") %></strong></a>
   <%
   }else{
    %>
    <input type="file" name="concentration_chart_filename" id="concentration_chart_filename" class="form-control" size="40" style="width:300px;padding-bottom:20px ;font-size: 12px !important;color: black;text-align: left !important;">
    <%	   
   }
   %>
    </td>
  ...
  <tr>
    <th>Is Measurement Variation Study required for this project</th>
    <td style="font-size: 12px;color: black;">
    <% 
    if(rs_checkMeasure.getInt("measureVariation_studyreq")!=0){
    if(rs_checkMeasure.getInt("measureVariation_studyreq")==1){ %>
    <select id="measureVariation_studyreq" name="measureVariation_studyreq" class="form-control" style="font-weight:bold; font-size: 12px !important;color: black;text-align: left !important;" >
    <option value="1">Yes</option>  
    </select> 
    <% 
    }else if(rs_checkMeasure.getInt("measureVariation_studyreq")==2){
    %>
    <select id="measureVariation_studyreq" name="measureVariation_studyreq" class="form-control" style="font-weight:bold; font-size: 12px !important;color: black;text-align: left !important;" >
     <option value="2">No</option> 
    </select> 
    ...
    <td>
    <%
    if(!rs_checkMeasure.getString("measureVar_studyreq_filename").equalsIgnoreCase("")){
    %>
    <input type="hidden" name="measureVar_studyreq_filename_flag" id="measureVar_studyreq_filename_flag" value="avail">
	<a href="SixSigma_MeasureDocsDisplay.jsp?field=<%=rs_checkMeasure.getInt("id")%>&cond=measureVariation_studyreq"><strong style="color: blue;font-weight: bold;font-size: 12px !important;"><%=rs_checkMeasure.getString("measureVar_studyreq_filename") %></strong></a>
    <%
    }else{
    %>
    <input type="file" name="measureVar_studyreq_filename" id="measureVar_studyreq_filename" class="form-control" size="30" style="width:300px; padding-bottom:20px ;font-size: 12px !important;color: black;text-align: left !important;">
    <%	
    }
    %>
    </td>
  ...
  <tr>
    <th>Tools used in the Measure Phase</th>
    <td colspan="2" align="left" style="color: black;font-size: 12px;">
    <%-- <% 
    String logTools="";
    ps_check = con_master.prepareStatement("select master_name from rel_pt_masterData where enable=1 and searchTerm='toolsMeasurePhase' and id in (select tools_used_id from rel_pt_measurePhase_tools where enable=1 and measurePhase_id="+rs_checkMeasure.getInt("id")+")");
    rs_check = ps_check.executeQuery();
    while(rs_check.next()){
    	logTools = rs_check.getString("master_name") + " , " + logTools; 
    %>
    <%
	}
    %>
    ...
    <%
    int cntMeasure=0;
    ps_check = con_master.prepareStatement("select * from rel_pt_masterData where enable=1 and searchTerm='toolsMeasurePhase' and id in "+
    		" (select tools_used_id from rel_pt_measurePhase_tools where enable=1 and measurePhase_id="+rs_checkMeasure.getInt("id")+")");
    rs_check = ps_check.executeQuery();
    while(rs_check.next()){
    	cntMeasure++;
	%>
    <input type="checkbox" checked="checked" onClick="return false;" name="tools_used" id="<%=cntMeasure%>"  value="<%=rs_check.getInt("id")%>">&nbsp;<%=rs_check.getString("master_name") %>&nbsp;&nbsp;
    <%
	}
     ps_check = con_master.prepareStatement("select * from rel_pt_masterData where enable=1 and searchTerm='toolsMeasurePhase' and id not in "+
    		" (select tools_used_id from rel_pt_measurePhase_tools where enable=1 and measurePhase_id="+rs_checkMeasure.getInt("id")+")");
    rs_check = ps_check.executeQuery();
    while(rs_check.next()){
    	cntMeasure++;
	%>
    <input type="checkbox" name="tools_used" id="<%=cntMeasure%>" onclick="selected_checkbox('<%=cntMeasure %>','<%=rs_check.getInt("id") %>')" value="<%=rs_check.getInt("id")%>">&nbsp;<%=rs_check.getString("master_name") %>&nbsp;&nbsp;
    <%
	}
    %>
  ...
  <tr>
  <td align="left" style="padding-left:400px;" colspan="3">
	<input type="submit" class="btn-primary" name="submit" id="submitMeasure" value=" SAVE " style="font-weight: bold;width: 100px;height: 25px;font-size: 12px;">
  </td>
</tr>
</table>
</form>
...
<input type="hidden" name="measurePID" id="measurePID" value="<%=problem_id%>">
<input type="hidden" name="measurePhaseID" id="measurePhaseID" value="<%=phaseID%>">
<input type="hidden" name="editMeasure" id="editMeasure" value="0">
<table class="gridtable" width="100%">
  <tr>
    <td style="font-size: 12px;font-weight: bold;background-color: #a59eff;" align="center">Measure Details</td>
    <td style="font-size: 12px;font-weight: bold;background-color: #a59eff;" align="center">Update</td>
  ...
  <tr>
    <th>Month in which the rejection is maximum</th>
    <td>
	<select id="month_maxRejection" name="month_maxRejection" class="form-control" style=" font-size: 12px !important;color: black;text-align: left !important;">
    <option value="">- - - Select Month - - - </option>
    <option value="<%= currMonth%>"><%= months[currMonth].toString()%></option>
    <%
    for (int i = 0; i < months.length-1; i++) {
    ...
    <select id="year_maxRejection" name="year_maxRejection" class="form-control" style="font-size: 12px !important;color: black;text-align: left !important;">
    <option value="">Select Year</option>
     <option value="<%=year%>"><%=year%></option>
    <%
    for(int i=year-4;i<=year+1;i++){
  ...
  <tr>
    <th>Minimum Rejection ppm</th>
    <td><input type="text" id="min_rejectPPM" name="min_rejectPPM" class="form-control" onkeypress="return validatenumerics(event);" style="font-size: 12px !important;color: black;text-align: left !important;"></td><td>&nbsp;</td>
  </tr>
  <tr>
    <th>Maximum Rejection ppm</th>
    <td><input type="text" name="max_rejectPPM" id="max_rejectPPM" class="form-control" onkeypress="return validatenumerics(event);" style="font-size: 12px !important;color: black;text-align: left !important;"></td><td>&nbsp;</td>
  </tr>
  <tr>
    <th>Number of Machines used for doing the Suspected Last Manufacturing Process</th>
    <td><input type="text" name="no_machinesUsed" id="no_machinesUsed" class="form-control" onkeypress="return validatenumerics(event);" style="font-size: 12px !important;color: black;text-align: left !important;"></td><td>&nbsp;</td>
  </tr>
  <tr>
    <th>Number of Streams in the Suspected Last Manufacturing Process</th>
    <td><input type="text" name="no_streams" id="no_streams" class="form-control" onkeypress="return validatenumerics(event);" style="font-size: 12px !important;color: black;text-align: left !important;"></td><td>&nbsp;</td>
   </tr>
  <tr>
    <th>Response</th>
    <td> 
  ...
  <tr>
	...
	</td><td><input type="file" name="abnormal_eqCond_fileName" id="abnormal_eqCond_fileName" class="form-control" size="40" style="width:300px;padding-bottom:20px ;font-size: 12px !important;color: black;text-align: left !important;"></td>
  </tr>
  <tr>
    <th>Are there any abnormalities in the Process parameters found during Audit</th>
    <td>
   <select id="abnormal_processParameters" name="abnormal_processParameters" class="form-control" style="font-weight:bold; font-size: 12px !important;color: black;text-align: left !important;" >
    ...
    <td><input type="file" name="abnormal_processPara_filename" id="abnormal_processPara_filename" class="form-control" size="40" style="width:300px;padding-bottom:20px ;font-size: 12px !important;color: black;text-align: left !important;"></td>
  </tr>
  <tr>
    <th>Is the Rejection Trend for 6 months plotted</th>
    <td>
    ...
    <td><input type="file" name="rejection_trnd6Month_fileName" id="rejection_trnd6Month_fileName" class="form-control" size="40" style="width:300px;padding-bottom:20px ;font-size: 12px !important;color: black;text-align: left !important;"></td>
  </tr>
  <tr>
    <th>Is Concentration chart applicable for the Project</th>
    <td>
    ...
    <td><input type="file" name="concentration_chart_filename" id="concentration_chart_filename" class="form-control" size="40" style="width:300px;padding-bottom:20px ;font-size: 12px !important;color: black;text-align: left !important;"></td>
  </tr>
  <tr>
    <th>Is Measurement Variation Study required for this project</th>
    <td>
    ...
    <td><input type="file" name="measureVar_studyreq_filename" id="measureVar_studyreq_filename" class="form-control" size="30" style="width:300px; padding-bottom:20px ;font-size: 12px !important;color: black;text-align: left !important;"></td>
  </tr>
  <tr>
    <th>Tools used in the Measure Phase</th>
    <td colspan="2" align="left" style="color: black;font-size: 12px;">
    <%
    int cntMeasure=0;
    ps_check = con_master.prepareStatement("select * from rel_pt_masterData where enable=1 and searchTerm='toolsMeasurePhase'");
    rs_check = ps_check.executeQuery();
    while(rs_check.next()){
    	cntMeasure++;
	%>
...
<table class="gridtable" width="100%">
  <tr>
    <th colspan="4">List of Suspected Sources of Variation (SSV's) </th>
  </tr>
  <tr>
  ...
  <tr>
  	<td>
  	<textarea class="form-control" name="ssv_text" id="ssv_text" rows="1" cols="30" onkeypress="return checkQuote();" style="color: black;font-size: 12px;"></textarea>
  	</td>
  	<td>
  	<%
    ps_check = con_master.prepareStatement("select * from rel_pt_masterData where enable=1 and searchTerm='ssv_measure'");
    rs_check = ps_check.executeQuery();
    %>
	...
	%>
  	...
  	<td>
  	<%
    ps_check = con_master.prepareStatement("select * from rel_pt_masterData where enable=1 and searchTerm='ssv_variation'");
    rs_check = ps_check.executeQuery();
    %>
	<select id="ssv_variation" name="ssv_variation" class="form-control" style="font-weight:bold; font-size: 12px !important;color: black;text-align: left !important;">
    <option value=""> - - Select - - </option>
	<%
	while(rs_check.next()){
	%>
	...
	%>
  	...
  	<td align="center"> 
  	<img alt="#" src="img/save.png" style="cursor: pointer;height: 25px;" id="ssvImage" onclick="javascript:saveSSVData('<%=problem_id%>','<%=phaseID%>')">  	
  	</td>
  </tr> 
</table>
</div>
<!-- ************************************************************************************************************************* -->
<!-- ************************************************************************************************************************* -->
<div style="width: 49%;overflow: scroll;float: right;height: 200px;">
<span id="ssvMeasure">
<table class="gridtable" width="100%">
  <tr>
  ...
  ps_data = con_master.prepareStatement("select * from rel_pt_measurePhase_SSV where enable=1 and phase_id="+phaseID+" and problem_id="+problem_id+" order by seq_no desc");
  rs_data = ps_data.executeQuery();
  while(rs_data.next()){

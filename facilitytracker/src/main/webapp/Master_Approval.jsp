...
/*   function decision_call(val) {
	...
	function sendMessage(code,val) {
		 ...
		 var hsn_no = document.getElementById('hsn_no'+val); 
		...
		if(cndCheck!=''){
		 ...
		 var xmlhttp;
			...
			xmlhttp.onreadystatechange = function() {
		 	...
		 	xmlhttp.open("POST", "Master_MSGSend.jsp?id=" + val + "&sendMsg=" + sendMsg.value + "&select=" + code + "&hsn_no=" + hsn_no.value, true);  
			xmlhttp.send(); 
		}else{
      ...
      <section class="wrapper">
        ...
        <div class="row">
          <div class="col-lg-12">
            ...
            <ol class="breadcrumb">
              <li><i class="icon_ribbon_alt"></i>Provide Approval</li>
             <!--  <li><i class="icon_box-checked" style="color: gray;"></i><a href="Master_Approval.jsp" style="font-weight: bold;">All Approvals</a>  </li> -->
              <li><i class="icon_refresh" style="color: gray;"></i><a href="Master_Approval.jsp" style="font-weight: bold;">Refresh</a>  </li>
        <%
...
<div class="row" style="height:500px;">
             ...
             <div class="table-responsive"> 
				<table style="width: 100%;" class="gridtable"> 
					<tr style="background-color: #dedede;color: black;text-align: center;"> 
						...
						<th>Required In</th>
						<th>Amount</th> 
						<th style="background-color: yellow;">HSN NO</th>
						<th>Reason</th> 
						<th>Call</th>
						<th>Overdue (Hrs)</th>
						<th>Status</th>
	...
	long diff;
	String query = "",hsn_code="";
	ps_check = con_master.prepareStatement("SELECT plant  FROM rel_SAPmaster_releaseStrategy where enable=1 and user_type='pur' and u_id="+uid);
 	rs_check = ps_check.executeQuery();
 	if(rs_check.next()){ 
	query = "SELECT * FROM tran_SAPmaster_create where enable=1 and plant in(SELECT plant  FROM rel_SAPmaster_releaseStrategy where enable=1 and user_type='pur' and u_id="+uid+") and enable=1 and stage_no=1  order by id desc";
 	}
 	
 	if(dept_id==18){
 		query = "SELECT * FROM tran_SAPmaster_create where enable=1 and stage_no=1  order by id desc";
 	}
	...
	<tr style="cursor:default; font-size: 12px;">
		...
		<td align="right"> 
		<%
		 if(rs_check.getString("hsn_code")!=null){
		 hsn_code = rs_check.getString("hsn_code");
		 }
		%> 
		<input class="form-control" style="font-weight: bold;color: black;font-size: 14px;" value="<%=hsn_code %>" onkeypress="return validatenumerics(event);" id="hsn_no<%=rs_check.getInt("id") %>" name="hsn_no<%=rs_check.getInt("id") %>" maxlength="8" type="text" minlength="4" />
		</td>
		
		<td><%=rs_check.getString("reason") %></td>
		<td align="right"><b><%=logBy %></b><br><%=rs_check.getString("contactNo") %></td>   
		<%  
		diff = Math.abs(date2cmp.getTime() - format2.parse(rs_check.getString("log_date")).getTime()) / (60 * 60 * 1000); 
		%> 
		<td align="center"><%=diff %></td>
		
		
		<td align="center"><a href="Master_Edit.jsp?id=<%=rs_check.getInt("id")%>&appr=1" style="font-weight: bold;color: blue;" title="Click to edit/Make corrections if any"><%=rs_check.getString("status_id") %></a></td> 
		...
		<img src="img/check.png" title="Approve" style="cursor: pointer;" onclick="sendMessage('2',<%=rs_check.getInt("id") %>); return false;" id="approve<%=rs_check.getInt("id") %>">&nbsp;
		<img src="img/close.png" title="Reject" style="cursor: pointer;" onclick="sendMessage('4',<%=rs_check.getInt("id") %>); return false;" id="reject<%=rs_check.getInt("id") %>">
		...
		<td>
		<a href="Master_Approval_Log.jsp?id=<%=rs_check.getInt("id")%>&home=0">

    ...
    <section id="main-content">
      ...
      <div class="row"> 
        ...
        <jsp:include page="SixSigma_Header.jsp" > 
  		<jsp:param name="define" value="<%=control %>" />
		</jsp:include> 
       </div> 
<!-- ********************************************************************************************************************* -->
<div style="height:500px;overflow: scroll;"> 
			<table class="gridtable" width="100%"> 
					...
					/* defineQuery = "select * from tran_pt_problem where phase_id=6 and enable=1 and project_lead=" +uid+
							" union select * from tran_pt_problem where phase_id=6 and id in "+
					...
					while(rs_data.next()){
						defineQuery = "select * from tran_pt_problem where enable=1 and phase_id=6 order by changed_date desc";
					}
					
					
					ps_data = con_master.prepareStatement("select * from tran_pt_problem where phase_id=6 and enable=1 and project_lead=" +uid);
					rs_data = ps_data.executeQuery();
					while(rs_data.next()){
						defineQuery = "select * from tran_pt_problem where enable=1 and phase_id=6 and project_lead=" +uid +" order by changed_date desc";
					}
...
<a href="SixSigma_Control_Data.jsp?problem_id=<%=rs_check.getInt("id")%>" style="color: black;" title="Update Data"><img alt="#" src="img/update.png" style="height: 20px;"> </a>

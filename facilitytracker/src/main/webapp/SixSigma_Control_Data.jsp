...
function validatenumerics(key) { 
  ...
  function decision_call(val) {
	...
	function numericEvent(val,input) {			    
		...
		var sum = parseInt(defineScore) + parseInt(measureScore) + parseInt(analyzeScore) + parseInt(improveScore); 
		if (val<input || input==0) {
		   document.getElementById('input_value').value = '';
		   document.getElementById('logScore').value = sum;
		   alert("Wrong Input");
		}else{
			sum = parseInt(measureScore) + parseInt(defineScore)+parseInt(input) + parseInt(analyzeScore) + parseInt(improveScore);
			document.getElementById('logScore').value = sum;
		} 
	}
	...
	function validateForm() {
		...
		var applicable_act = document.getElementById("applicable_act").value;			    
		...
		if(applicable_act ==""){
			alert("Is Variation Analysis Applicable...!!!");
			document.getElementById("submitMeasure").disabled = false;
			return false;
		}
		var flag=false;
		...
		if(flag==false){
			alert("Type of Control method implemented....?");
			document.getElementById("submitMeasure").disabled = false;
			return false;
		}
		/*if(analyze_filename ==""){
			alert("Supporting documents...?");
			document.getElementById("submitMeasure").disabled = false;
			return false;
		}
		 if(dp5 ==""){
      ...
      <section class="wrapper">
       ...
       <div class="row"> 
        ...
        <jsp:include page="SixSigma_Header.jsp" > 
  		<jsp:param name="define" value="<%=control_data %>" />
		</jsp:include> 
        </div>   
<!-- ********************************************************************************************************************* -->
 <div>
...
<table class="gridtable" width="100%">
  <tr>
    ...
    <td align="center" style="font-size: 12px;font-weight: bold;background-color: #a59eff">CONTROL PHASE</td>
      	<%
  		if(request.getParameter("success")!=null){
		%>
		<td align="center" style="font-size: 12px;font-weight: bold;background-color: #06a415;color: white;">
		<a href="SixSigma_Control_Data.jsp?problem_id=<%=problem_id%>" style="color: white;"><%=request.getParameter("success") %></a>
		</td>
        <%
		...
		<a href="SixSigma_Control_Data.jsp?problem_id=<%=problem_id%>"  style="color: white;"><%=request.getParameter("statusNop") %></a>
		</td>  
	    <%
		}
		%>
...
while(rs_sql.next()){
	...
	String control_days ="" ,improve_days="",result_days="";
	int control_lbl =0;
...
<img src="img/LogHere.png" title="History" style="cursor: pointer;height: 25px;">
  ...
  <tr>
   ...
   <%
   	int totalSum =0,defineScore=0,   measureScore=0,   improveScore=0,controlScore=0;
  ...
  <% 
    ...
    ps_check1 = con_master.prepareStatement("select * from rel_pt_dateTimeline where phase_id=5 and enable=1 and problem_id="+problem_id);
    rs_check1 = ps_check1.executeQuery();
    while(rs_check1.next()){
    	result_days = sdf.format(rs_check1.getDate("plan_endDate"));
    %>
  <tr>
  ...
  <% 
   ...
   %> 
	...
	ps_data = con_master.prepareStatement("select actual_score from rel_pt_dateTimeline where enable=1 and phase_id=6 and problem_id="+problem_id);
	rs_data = ps_data.executeQuery();
	if(rs_data.next()){
		controlScore = rs_data.getInt("actual_score"); 
	}
    ...
    rs_check1 = ps_check1.executeQuery();
    if(rs_check1.next()){
    	ps_data = con_master.prepareStatement("select actual_score,act_startDate,act_endDate from rel_pt_dateTimeline where enable=1 and (phase_id=6 or phase_id=7) and problem_id="+problem_id);
    	rs_data = ps_data.executeQuery();
    	if(rs_data.next()){
   	...
   	%>
    	<input type="text" name="input_value" id="input_value" value="<%=inputValue %>" onkeypress="return validatenumerics(event);" onkeyup="numericEvent(<%= control_lbl %>,this.value)" maxlength="3" minlength="1" class="form-control" style="height: 23px !important;width: 60px;color: black;font-weight: bold;text-align: right;" required>
   <%
    	}else{
    	ps_data = con_master.prepareStatement("select id from rel_pt_reviewer where enable=1 and u_id="+uid);
		rs_data = ps_data.executeQuery();
		if(rs_data.next()){
		    %>
		<input type="text" name="input_value" id="input_value"  onkeypress="return validatenumerics(event);" onkeyup="numericEvent(<%= control_lbl %>,this.value)" maxlength="3" minlength="1" class="form-control" style="height: 23px !important;width: 60px;color: black;font-weight: bold;text-align: right;" required>
		    <%
			}
   		  }
    	}
    totalSum = defineScore + measureScore + analyzeScore + improveScore + controlScore;
    %>
	</td>
    <td style="color: black;" align="right"><%=result_days %></td>
    <td style="color: black;" align="right"><%=control_days%></td>
...
<div>
<form action="SixSigma_Control" method="post" enctype="multipart/form-data" onSubmit="return validateForm();">
...
<table class="gridtable" width="100%">
 ...
 <tr>
 	...
 	<th style="background-color: #0b2f50;color: white;">Is Variation Analysis Applicable</th>
 	<th style="background-color: #0b2f50;color: white;">Type of Control method implemented</th>
 	<th style="background-color: #0b2f50;color: white;">Supporting documents</th>
  ...
  <tr>
  	<td>
  	<%
  	ps_data = con_master.prepareStatement("select rel_pt_improvePhase_Causes.id,variation_source,improvement_action,image_name,image_attach,completion_date,resp_name from "+
  			" rel_pt_improvePhase_Causes inner join rel_pt_measurePhase_SSV on rel_pt_measurePhase_SSV.id= "+
  			...
  			" and rel_pt_improvePhase_Causes.id not in (select improveCause_id from rel_pt_controlPhase_Causes where enable=1 and problem_id="+problem_id+")");
	rs_data = ps_data.executeQuery(); 
    %>
	...
	while(rs_data.next()){
	%>
	<option value="<%=rs_data.getInt("id")%>"><%=rs_data.getString("variation_source") %></option>
	<%
	}
	%>
	</select>
  	...
  	<select id="applicable_act" name="applicable_act" class="form-control" style="font-weight:bold; font-size: 12px !important;color: black;text-align: left !important;">
    	<option value=""> Select </option>
	...
	<%
    int cntMeasure=0;
	ps_data = con_master.prepareStatement("select * from rel_pt_masterData where enable=1 and searchTerm ='controlMethod'");
	rs_data = ps_data.executeQuery(); 
	while(rs_data.next()){
    cntMeasure++;
    %> 
    <input type="checkbox" style="display: inline;-moz-transform: scale(1.5);margin-left: 10px;" name="ssv_pinpointing" id="<%=cntMeasure%>" onclick="selected_ssvPinpointing('<%=cntMeasure %>','<%=rs_data.getInt("id") %>')" value="<%=rs_data.getInt("id")%>">&nbsp;&nbsp;<span style="font-size: 12px;"> <%=rs_data.getString("master_name") %></span>&nbsp;&nbsp;
    <br>
    <%
	}
    %>
	...
	<td>
	<input type="file" name="control_filename" id="control_filename" class="form-control" size="30" style="width:150px; font-size: 12px !important;color: black;text-align: left !important;">
	</td>
	<td>
	<!-- <span class="input-append date" id="dpYears" data-date-format="dd-mm-yyyy" data-date-viewmode="years" style="cursor: pointer;">
   		<input class="form-control" id="dp5" name="confirm_completeDate"  size="16" type="text" onkeypress="return checkQuote();" style="color: black;width: 150px;font-size: 12px;cursor: pointer;">
   	<span class="add-on" style="visibility: hidden;margin: 5px;"><i class="icon-calendar"></i></span>
 	</span> -->
...
<div>
<br>
<form action="SixSigma_Control" method="post" enctype="multipart/form-data" onsubmit="submitCause.disabled = true; return true;">
<input type="hidden" name="measurePID" id="measurePID" value="<%=problem_id%>">
<input type="hidden" name="measurePhaseID" id="measurePhaseID" value="<%=phaseID%>">
...
<table class="gridtable" width="100%">
  ...
  ps_data = con_master.prepareStatement("select * from rel_pt_controlPhase_Causes where problem_id=" + problem_id + " and enable=1");
  rs_data = ps_data.executeQuery();
  while(rs_data.next()){
  %>
  <tr style="font-size: 12px;">
	...
	<%
	ps_data2 = con_master.prepareStatement("select variation_source from rel_pt_measurePhase_SSV where problem_id="+problem_id+" and id in("+
			"select analyzeSSV_id from rel_pt_improvePhase_Causes where problem_id="+problem_id+" and id="+rs_data.getInt("improveCause_id")+")");
	rs_data2 = ps_data2.executeQuery();
	while(rs_data2.next()){  
	%>
	<%=rs_data2.getString("variation_source") %>
	<%	
	}
	%>
	</td>
  	<td style="font-size: 12px;color: black;"><%if(rs_data.getInt("applicable_act")==1){%>Yes<%}else{%>No<%} %></td>
  	<td style="font-size: 12px;color: black;">
 <%
  	ps_data2 = con_master.prepareStatement("select * from rel_pt_masterData where enable=1 and searchTerm='controlMethod' and id in "+
  			"(select control_method_id from rel_pt_controlPhase_Causes_crtMethod where problem_id="+problem_id+" and controlPhaseCauses_id="+rs_data.getInt("id")+" and enable=1)");
	rs_data2 = ps_data2.executeQuery();
	while(rs_data2.next()){ 
	%>
	...
	<%	
  	...
  	<% 
  	if(!rs_data.getString("control_filename").equalsIgnoreCase("")){
  		flag_file="avail";
  	%> 
  	<a href="SixSigma_MeasureDocsDisplay.jsp?field=<%=rs_data.getInt("id")%>&cond=controlAttach"><strong style="color: blue;font-weight: bold;"><%=rs_data.getString("control_filename") %></strong></a> 
  	<%
  	}else{
  	%>
  	<input type="file" name="control_filename<%=rs_data.getInt("id") %>" id="control_filename<%=rs_data.getInt("id") %>" class="form-control" size="30" style="width:150px; font-size: 12px !important;color: black;text-align: left !important;">
	<%	
  	}
  	%>

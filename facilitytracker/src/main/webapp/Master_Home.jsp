    ...
    <section id="main-content">
      <section class="wrapper">
        ...
        <div class="row"> 
            ...
            <ol class="breadcrumb">
              <li><i class="icon_ribbon_alt"></i>My Requests</li>
             <li><i class="icon_pencil" style="color: gray;"></i><a href="Master_Create.jsp" style="font-weight: bold;">Create New</a></li>
              ...
              <li><i class="icon_refresh" style="color: gray;"></i><a href="Master_Home.jsp" style="font-weight: bold;">Refresh</a>  </li>
        <%
...
<div class="row" style="height:500px;">
	...
	ps_check = con_master.prepareStatement("SELECT plant  FROM rel_SAPmaster_releaseStrategy where enable=1 and u_id="+uid);
 	...
 	if(rs_check.next()){
	query = "SELECT * FROM tran_SAPmaster_create where plant in(SELECT plant  FROM rel_SAPmaster_releaseStrategy where enable=1 and user_type='pur' and u_id="+uid+") or created_by="+uid+" and enable=1 and stage_no!=0 and stage_no!=4  order by id desc";
 	}else{
 		query = "SELECT * FROM tran_SAPmaster_create where enable=1 and created_by="+uid+" and stage_no!=0 and stage_no!=4  order by id desc";
 	}
	
 	 if(dept_id==18){
  		query = "SELECT * FROM tran_SAPmaster_create where enable=1 and stage_no!=0 and stage_no!=4  order by id desc";
  	}
	...
	<tr style="cursor:default; font-size: 12px;">
		...
		<td width="10%">Req. Date : <%=format.format(rs_check.getDate("created_date")) %>,<br><b><%=rs_check.getString("reason") %></b></td>  
		<td align="right"><b><%=logBy %></b><br><%=rs_check.getString("contactNo") %></td>
		<%
		if(rs_check.getInt("stage_no")==1){
		%>
		<td align="center"><a href="Master_Edit.jsp?id=<%=rs_check.getInt("id")%>&appr=0" style="font-weight: bold;color: blue;" title="Click to edit/Make corrections if any"><%=rs_check.getString("status_id") %></a></td> 
		<%
		}else{
		%>
		...
		if(rs_check.getInt("stage_no")==1){
		%>
		<img src="img/close.png" title="Reject" style="cursor: pointer;" onclick="decision_call(<%=rs_check.getInt("id") %>); return false;" >
		<%
		}else{
		%>
		-
		...
		<td>
		<a href="Master_Approval_Log.jsp?id=<%=rs_check.getInt("id")%>&home=1">

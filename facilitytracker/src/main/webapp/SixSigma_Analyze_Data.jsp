...
function validatenumerics(key) { 
  ...
  function decision_call(val) {
	...
	function numericEvent(val,input) {			    
		...
		if (val<input || input==0) {
		   ...
		   alert("Wrong Input");
			...
			sum = parseInt(measureScore)+parseInt(defineScore)+parseInt(input);
			document.getElementById('logScore').value = sum;
		}
	}
	...
	function validateForm() {
		...
		if(ssv_measure ==""){
			alert("Please Provide Suspected Sources of Variation...!!!");
			document.getElementById("submitMeasure").disabled = false;
			return false; 
		}
		if(ssv_confirm ==""){
			alert("Is it Confirmed or Eliminated...!!!");
			document.getElementById("submitMeasure").disabled = false;
			return false; 
		}
		if(flag==false){
			alert("Please select Tool used for Pinpointing/Eliminating");
			document.getElementById("submitMeasure").disabled = false;
			return false;
		}
...
<!--main content start-->
    <section id="main-content">
      <section class="wrapper">
        ...
        <div class="row"> 
        <%
        String analyze_data="analyze_data";
        %>
        <jsp:include page="SixSigma_Header.jsp" > 
  		<jsp:param name="define" value="<%=analyze_data %>" />
		</jsp:include>
        </div> 
<!-- ********************************************************************************************************************* -->
  <div>
...
<table class="gridtable" width="100%">
  <tr>
    ...
    <td align="center" style="font-size: 12px;font-weight: bold;background-color: #a59eff">ANALYZE PHASE</td>
      	<%
...
while(rs_sql.next()){
	...
	String analyze_days ="",improve_days ="",control_days ="",measure_days="";
	int analyze_lbl =0,improve_lbl =0,control_lbl =0;
...
<img src="img/LogHere.png" title="History" style="cursor: pointer;height: 25px;">
  ...
  <%
    ...
    rs_check1 = ps_check1.executeQuery();
    while(rs_check1.next()){
    	measure_days = sdf.format(rs_check1.getDate("plan_endDate"));
    %>
  <tr>
  ...
  %>
    ...
    rs_check1 = ps_check1.executeQuery();
    if(rs_check1.next()){
    	ps_data = con_master.prepareStatement("select actual_score,act_startDate,act_endDate from rel_pt_dateTimeline where enable=1 and (phase_id=4 or phase_id=5) and problem_id="+problem_id);
    	rs_data = ps_data.executeQuery();
    	if(rs_data.next()){
   	...
   	%>
    	<input type="text" name="input_value" id="input_value" value="<%=inputValue %>" onkeypress="return validatenumerics(event);" onkeyup="numericEvent(<%= analyze_lbl %>,this.value)" maxlength="3" minlength="1" class="form-control" style="height: 23px !important;width: 60px;color: black;font-weight: bold;text-align: right;" required>
   <%
    	}else{
    	ps_data = con_master.prepareStatement("select id from rel_pt_reviewer where enable=1 and u_id="+uid);
		rs_data = ps_data.executeQuery();
		if(rs_data.next()){
		    %>
		<input type="text" name="input_value" id="input_value"  onkeypress="return validatenumerics(event);" onkeyup="numericEvent(<%= analyze_lbl %>,this.value)" maxlength="3" minlength="1" class="form-control" style="height: 23px !important;width: 60px;color: black;font-weight: bold;text-align: right;" required>
		    <%
			}
   		  }
    	}
    totalSum = defineScore + measureScore + analyzeScore;
    %>
	</td>
...
<!-- ************************************************************************************************************************* -->
<div>
<form action="SixSigma_AnalyzeControl" method="post" enctype="multipart/form-data" onSubmit="return validateForm();">
...
<table class="gridtable" width="100%">
  <tr>
    <th colspan="5" style="background-color: #A59EFF;color: black;">List of Suspected Sources of Variation (SSV's) </th>
  </tr>
  <tr>
    <th style="background-color: #0b2f50;color: white;">Description</th>
    <th style="background-color: #0b2f50;color: white;">Is it Confirmed or Eliminated</th>
    <th style="background-color: #0b2f50;color: white;">Tool used for Pinpointing/Eliminating</th>
    <th style="background-color: #0b2f50;color: white;">Support Document(If Any)</th>
    <th style="background-color: #0b2f50;color: white;">Save</th>
  </tr>
  <tr>
  	<td>
  	<%
  	ps_check = con_master.prepareStatement("select * from rel_pt_measurePhase_SSV where enable=1 and phase_id=3 and problem_id="+problem_id+" and id not in "+
  		" (select measurePhaseSSV_id from rel_pt_analyzePhase_SSV where problem_id="+problem_id+" and enable=1)");
    rs_check = ps_check.executeQuery();
    %>
	...
	%>
  	...
  	<td>
  	<%
    ps_check = con_master.prepareStatement("select * from rel_pt_masterData where enable=1 and searchTerm='confirm'");
    rs_check = ps_check.executeQuery();
    %>
	<select id="ssv_confirm" name="ssv_confirm" class="form-control" style="font-weight:bold; font-size: 12px !important;color: black;text-align: left !important;">
    <option value=""> - - Select - - </option>
	<%
	while(rs_check.next()){
	%>
	...
	<td>
	<%
    int cntMeasure=0,ctR=2;
    ps_check = con_master.prepareStatement("select * from rel_pt_masterData where enable=1 and searchTerm='tools_pinpointing'");
    rs_check = ps_check.executeQuery();
    while(rs_check.next()){
    	cntMeasure++;
    	if(ctR==0){
	...
	<td>
	<input type="file" name="analyze_filename" id="analyze_filename" class="form-control" size="30" style="width:300px; font-size: 12px !important;color: black;text-align: left !important;">
	</td>
  	<td align="center">
...
<table class="gridtable" width="100%">
  ...
  <%
  String variation_source="",confirm="";
  ps_data = con_master.prepareStatement("select * from rel_pt_analyzePhase_SSV where enable=1 and phase_id="+phaseID+" and problem_id="+problem_id);
  rs_data = ps_data.executeQuery();
  while(rs_data.next()){
	  ps_data1 = con_master.prepareStatement("select variation_source from rel_pt_measurePhase_SSV where id="+rs_data.getInt("measurePhaseSSV_id"));
	  rs_data1 = ps_data1.executeQuery();
	  while(rs_data1.next()){
		  variation_source = rs_data1.getString("variation_source");
	  }
	  ps_data1 = con_master.prepareStatement("select master_name from rel_pt_masterData where id="+rs_data.getInt("analyze_confirm_id"));
	  rs_data1 = ps_data1.executeQuery();
	  while(rs_data1.next()){
		  confirm = rs_data1.getString("master_name");
	  }
  %>
  <tr>
  <td style="font-size: 12px;color: black;"><%=variation_source %></td>
  <td style="font-size: 12px;color: black;"><%=confirm %></td> 
  <td style="font-size: 12px;color: black;">
  	<% 
    String logTools="";
    ps_check = con_master.prepareStatement("select master_name from rel_pt_masterData where enable=1 and searchTerm='tools_pinpointing' and id in (select analyze_toolsUsed_id from rel_pt_analyzePhaseTools_SSV where enable=1 and analyzePhase_SSV_id="+rs_data.getInt("id")+")");
    rs_check = ps_check.executeQuery();
    while(rs_check.next()){
  ...
  <td style="width: 250px;color: black;font-size: 12px;">
  <a href="SixSigma_MeasureDocsDisplay.jsp?field=<%=rs_data.getInt("id")%>&cond=tools_pinpointing"><strong style="color: blue;font-weight: bold;"><%=rs_data.getString("file_name") %></strong></a>
  </td>
  </tr>
  <%
  }

    ...
    <section id="main-content">
      ...
      <div class="row"> 
        ...
        <jsp:include page="SixSigma_Header.jsp" > 
  		<jsp:param name="define" value="<%=result %>" />
		</jsp:include> 
        </div> 
<!-- ********************************************************************************************************************* -->
<div style="height:500px;overflow: scroll;"> 
			<table class="gridtable" width="100%"> 
					...
					/* defineQuery = "select * from tran_pt_problem where phase_id=7 and enable=1 "+
							" and id not in (select problem_id from rel_pt_resultPhase where enable=1) and project_lead=" +uid+
							" union select * from tran_pt_problem where phase_id=7 and id in "+
							...
							" and id not in (select problem_id from rel_pt_resultPhase where enable=1)"; 
					...
					while(rs_data.next()){
						defineQuery = "select * from tran_pt_problem where enable=1 and phase_id=7 and id not in (select problem_id from rel_pt_resultPhase where enable=1) order by changed_date desc";
					}
					*/
					...
					ps_data = con_master.prepareStatement("select * from tran_pt_problem where phase_id=7 and enable=1 and project_lead=" +uid);
					rs_data = ps_data.executeQuery();
					while(rs_data.next()){ 
						defineQuery = "select * from tran_pt_problem where enable=1 and phase_id=7 and project_lead=" +uid +" and id not in (select problem_id from rel_pt_resultPhase where enable=1) order by changed_date desc";
					}
					...
					%>
					<tr>
						<td align="right" title="Logged By <%=rs_check.getString("created_byName")%>" style="cursor: pointer;"><%=rs_check.getInt("id") %></td>
...
<a href="SixSigma_Result_Data.jsp?problem_id=<%=rs_check.getInt("id")%>" style="color: black;" title="Update Data"><img alt="#" src="img/update.png" style="height: 20px;"> </a>

...
try{
	...
	DateFormat susFormat = new SimpleDateFormat("yyyy-MM-dd");
	DecimalFormat dfloat = new DecimalFormat("##0");
...
<!--main content start-->
    <section id="main-content">
      <section class="wrapper">
       ...
       <div class="row"> 
        <%
        String result_data="sustPlan_data";
        %>
        <jsp:include page="SixSigma_Header.jsp" > 
  		<jsp:param name="define" value="<%=result_data %>" />
		</jsp:include> 
       </div>   
<!-- ********************************************************************************************************************* -->
 <div> 
...
<table class="gridtable" width="100%">
  <tr>
    ...
    <td align="center" style="font-size: 12px;font-weight: bold;background-color: #a59eff">SUSTAIN PHASE</td>
      	<%
  		if(request.getParameter("success")!=null){
		%>
		<td align="center" style="font-size: 12px;font-weight: bold;background-color: #06a415;color: white;">
		<a href="SixSigma_Sustain_Data.jsp?problem_id=<%=problem_id%>" style="color: white;"><%=request.getParameter("success") %></a>
		</td>
        <%
		...
		<a href="SixSigma_Sustain_Data.jsp?problem_id=<%=problem_id%>" style="color: white;"><%=request.getParameter("statusNop") %></a>
		</td>  
	    <%
		}
		%>
...
while(rs_sql.next()){
	...
	ps_sql = con_master.prepareStatement("select * from rel_pt_phase where enable=1 and seqNo between 2 and 9");
	rs_sql = ps_sql.executeQuery();
	while(rs_sql.next()){ 
		if(rs_sql.getInt("seqNo")==9){
...
<img src="img/LogHere.png" title="History" style="cursor: pointer;height: 25px;">
			...
			<!-- Modal -->
                <div class="modal fade" id="myModaltimeline<%=problem_id %>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                  <div class="modal-dialog">
					...
					<table class="gridtable" width="100%"> 
                    <tr style='font-size: 12px; background-color: #fbffd4; border-width: 1px; padding: 8px; border-style: solid; border-color: #729ea5; text-align: center;'> 
						<td colspan='4' align="center"  style="background-color: #feffff;color: black;font-size: 12px;">Timeline Datewise Score</td>
					</tr> 
  ...
  <%
  totalSum = totalSum + rs_check1.getInt("actual_score");
	} 
  ...
  totalSum = totalSum + rs_check1.getInt("actual_score");
    ...
    ps_check1 = con_master.prepareStatement("select * from rel_pt_dateTimeline where phase_id=6 and enable=1 and problem_id="+problem_id);
    rs_check1 = ps_check1.executeQuery();
    while(rs_check1.next()){
    	controlDate = rs_check1.getDate("act_endDate"); 
    %>
  <tr>
  ...
  <tr>
    ...
    <td align="right" style="font-size: 12px;font-weight: bold;"><%=totalSum %></td>
    <td colspan="4">&nbsp;</td> 
  </tr>
</table>
</form>
...
<!-- ***************************************************** Result ***************************************************** -->
<div>  
<form action="SixSigma_ResultControl" method="post" onSubmit="return validateForm();">
<input type="hidden" name="hid_problem_id" id="hid_problem_id" value="<%=problem_id%>">
<input type="hidden" name="hid_phase_id" id="hid_phase_id" value="<%=phaseID%>"> 
<%
  	String[] months = new DateFormatSymbols().getMonths();
  	Calendar calSustain = Calendar.getInstance();
  	calSustain.setTime(controlDate);
  	...
  	int monthGet = cal.get(Calendar.MONTH);
  	String entryMonth = months[monthGet].toString();
  	
 
  	
	flag=false;
	ps_data2 = con_master.prepareStatement("select month_no,year_no from rel_pt_resultPhase where id="+
			"(select MAX(id) as id from rel_pt_resultPhase where phase_id="+phaseID+" and problem_id="+problem_id+" and enable=1)");
	rs_data2 = ps_data2.executeQuery();
	while(rs_data2.next()){
		if(rs_data2.getString("month_no")!=null){
			if(rs_data2.getInt("month_no")==11){
		  		month=0;
		  		year++;
		  		flag=true;
		  	}else{
				month = rs_data2.getInt("month_no"); 
				year=rs_data2.getInt("year_no");
				month++;
				flag=true;
		  	}
		} 
	...
	int cnt_mnt=0;
	ps_data2 = con_master.prepareStatement("select COUNT(id) as cnt from rel_pt_resultPhase where enable=1 and problem_id="+problem_id+" and phase_id=9");
	rs_data2 = ps_data2.executeQuery();
	while(rs_data2.next()){
		cnt_mnt=rs_data2.getInt("cnt");
	}
	if(cnt_mnt<12){
  	%>
<table class="gridtable" width="100%">
 <tr>
    <th colspan="7" style="font-size: 12px;font-weight: bold;background-color: #a59eff">Results</th>
  </tr>
  <tr>
  	<th>Month</th>
    <th>Base line Rejection ppm</th>
    <th>Targetted Rejection ppm</th>
    <th>Actual Achieved in ppm</th>
    <th>Planned Savings in Rs. Lakhs</th>
    <th>Actual Savings in Rs. Lakhs</th>
    <th>Save</th>
  </tr>
  <%-- <%
	System.out.println(entryMonth + " = = " + months[month].toString() );
  %> --%>
  <tr>  
  	<td>
  	<input type="hidden" name="controlDate" id="controlDate" value="<%=susFormat.format(calendar.getTime())%>">
  	<strong style="font-size: 12px;color: black;"><%=months[month].toString() %></strong> </td>
  	<td align="right" style="font-size: 12px;">
  	<input type="text" class="form-control" readonly="readonly" style="font-size: 12px;color: black;text-align:right;" value="<%=dfloat.format(baselinePPM) %>" name="baseLineReject" id="baseLineReject">
  	</td>
    <td align="right" style="font-size: 12px;">
    <input type="text" class="form-control" readonly="readonly" style="font-size: 12px;color: black;text-align:right;" value="<%=dfloat.format(baselinePPM_target) %>" name="baseLineReject_target" id="baseLineReject_target">
    </td>
    <td>
      <input type="text" class="form-control" style="font-size: 12px;color: black;text-align:right;" name="actualAchievedPPM" id="actualAchievedPPM" onkeypress="return validatenumerics(event);" required>
    </td>
    <td>
      <input type="text" class="form-control" readonly="readonly" style="font-size: 12px;color: black;text-align:right;" value="<%=expectedSaving %>" name="expectedSaving" id="expectedSaving">
    </td>
    <td>
      <input type="text" class="form-control" style="font-size: 12px;color: black;text-align:right;" name="actualSaving" id="actualSaving" onkeypress="return validatenumerics(event);" required>
    </td>
    <td>
    <%
    if(entryMonth.equalsIgnoreCase(months[month].toString())){
    %>
    <input type="submit" class="btn-primary" name="submit" id="submitMeasure" value="SAVE" style="font-size:12px; font-weight: bold;height: 24px !important;">
    <%
    }
    %>
    </td>
...
<table class="gridtable" width="100%">
 <tr>
    <th colspan="8" style="font-size: 12px;font-weight: bold;background-color: #b56751">Monthwise Results Uploaded</th>
  </tr>
  <tr>
  	<th>S.No</th>
  	<th>Month</th>
  ...
  <%
  int no=0;
	ps_data2 = con_master.prepareStatement("select * from rel_pt_resultPhase where  phase_id="+phaseID+" and enable=1 and problem_id="+problem_id +" order by id desc");
	rs_data2 = ps_data2.executeQuery();
	while(rs_data2.next()){
		no++;
  %>
  <tr align="right">
  	<td style="font-size: 12px;color: black;" align="center"><%=no %></td>
  	<td style="font-size: 12px;color: black;" align="left"><%=rs_data2.getString("month_year") %></td>
  	<td style="font-size: 12px;color: black;"><%=rs_data2.getInt("basline_rejectionPPM") %></td>
  	<td style="font-size: 12px;color: black;"><%=rs_data2.getInt("targetted_rejectionPPM") %></td>
  	<td style="font-size: 12px;color: black;"><%=BigDecimal.valueOf(rs_data2.getDouble("actual_achieved_PPM")) %></td>
  	<td style="font-size: 12px;color: black;"><%=BigDecimal.valueOf(rs_data2.getDouble("plan_savingRs")) %></td>
  	<td style="font-size: 12px;color: black;"><%=BigDecimal.valueOf(rs_data2.getDouble("actual_savingRs")) %></td>
  	<td style="font-size: 12px;color: black;"><%=format.format(rs_data2.getDate("changed_date")) %></td>
  </tr>
  <%
	}
  %>
